...
- ~~暂没拉股息率 / 派息~~ → 已实现，见 §15.8 `get_dividend_history`（双算法）
- 还没拉 **ROE / 营收同比 / 净利同比** 单独工具（部分能从 `_fhps_em_rows` 副产物拿到）
- 美股没正经个股数据，agent 只能按 ticker 名透传 + 训练知识
- 最近重要公告/新闻、行业研报等 → 走 §21 web_search

注：宏观 / 板块 / 资产类话题（港股 / 黄金 / 煤炭 / AI 等）由独立的「主题市场上下文」机制处理 —— 见 §16。

### 15.8 `get_dividend_history` —— A 股分红 + 双算法股息率

`bigv-market.get_dividend_history(query, last_n)`。比起 `get_stock_snapshot` 只给
PE/PB/市值，分红是独立的一套数据 + 算法，单独成工具。

数据源：`akshare.stock_history_dividend_detail`（新浪）+ `stock_fhps_detail_em`
（东财）。后者同时提供 **报告期、每股分红、每股收益、方案进度** —— 一次拉就够。

返回三类信息：

| 字段 | 含义 |
|------|------|
| `history[]` | 最近 N 条原始分红事件（公告日期 / 派息 / 除权日 / 进度） |
| `by_fiscal_year[]` | 按财年分组：H1 + 年报 分别派息额 + 全年 EPS + 派息率 + 是否已公告 |
| `algorithm_1_historical` | **历史口径股息率**（见下方）|
| `algorithm_2_forecast` | **预测口径股息率**（见下方）|

#### 算法 1（历史口径）

```
选最近一个已公告完成的财年（年报至少决议通过），
该 FY 总分红 (中报 + 年报) / 当前股价 = 历史股息率
```

代表「**按上一财年实际派息算的当前买入收益率**」。这是大多数投资者口里
"股息率"的标准定义。中国平安 FY2025 例：(0.95 + 1.75) / 53.76 = **5.02%**。

#### 算法 2（预测口径，仅供参考）

```
1. 各年派息率 = 当年分红/股 ÷ 当年 EPS_FY    （用近 3 年，最少 2 年）
2. 平均派息率
3. 各年 EPS YoY 增长率
4. 平均增长率
...
代表「**假设公司维持过去派息率 + EPS 增速**时，下一年的预期股息率」。中国平安例：
派息率均值 40.3% × 预测 EPS 9.80 / 53.76 = **7.35%**。

输出会自动 flag「一次性损益警告」—— 任一年 EPS YoY 波动 > 40% 时，note 字段
提示外推会被异常年份放大，agent 必须复述这条警告。

#### 为什么不只算一个

- 算法 1 太过保守：忽略公司盈利增长趋势
- 算法 2 太过乐观：把保险、银行这种容易有一次性投资收益波动的行业拉偏
- **两个都给 + 完整推导** → 用户看到全貌自己判断

agent 在分红问题上**必调此工具**且**两个算法都必须展示**（含 calculation 字段
逐步推导）。advisor / bigv 博主 / master 都遵守此规则 —— 博主分身可以另加自己
方法论的解读，但**不替代**工具的两个数字。规则写在 `openclaw/agents/advisor/
AGENTS.md` + `web/chat.py` 的 `system_prompt_for` 里。
...
```
   ...
   │ search/persona   │  │ get_stock_snapshot           │  │ headless 浏览器  │
   │ get_recent/post  │  │ get_dividend_history (双算法)│  │ 抓单 URL 完整内容│
   │ list_bloggers    │  │ get_market_context           │  │                  │
   │ search_multi_q   │  │ web_search (Bing CN)         │  │                  │
   └──────────────────┘  └──────────────────────────────┘  └──────────────────┘
   (仅 bigv 可用)        (bigv + advisor 都可用 — Tier 1+2) (仅 advisor 可用 — Tier 3)
```

要点：
- `bigv` 同时连 `bigv-blogger`（语料）+ `bigv-market`（行情 + web_search）
- `advisor` 连 `bigv-market`（行情 + web_search）+ `agent-browser`（深度抓页）——
  **禁止**碰 `bigv-blogger`
- `bigv-market` 是**共享**的：snapshot / dividend / market_context / web_search
  四个工具都对两 agent 开放（行情 + 通用搜索都跟博主语料无关）
- **三层数据架构**：Tier 1 = 结构化 MCP / Tier 2 = web_search / Tier 3 = agent-browser，
  详见 §21
...
- `bigv-twins-blogger.service`（port 8770）—— 博主专用
  - `list_bloggers / search / search_multi_query / get_persona / get_recent / get_post`
  - 资源：`persona://blogger/{slug}`
- `bigv-twins-market.service`（port 8771）—— 通用行情 + 搜索
  - `get_stock_snapshot / get_dividend_history / get_market_context / web_search`
...
- 数据来自三层架构（详见 §21）：
  - **Tier 1**：`bigv-market` 结构化 MCP（snapshot / dividend / market_context）
  - **Tier 2**：`bigv-market.web_search`（Bing CN，覆盖财报 / 业绩 / 新闻 / 政策）
  - **Tier 3**：`agent-browser`（深度抓单 URL，snippet 不够时兜底）
...
| 文件                                                | 职责                                                   |
| --------------------------------------------------- | ------------------------------------------------------ |
| `src/bigv_twins/blogger_server.py`                  | 博主语料 MCP server（FastMCP，streamable-http，:8770）<br>暴露 `search / search_multi_query / get_recent / get_post / get_persona / list_bloggers` |
| `src/bigv_twins/market_server.py`                   | 市场数据 MCP server（FastMCP，streamable-http，:8771）<br>暴露 `get_stock_snapshot / get_dividend_history / get_market_context / web_search` |
| `src/bigv_twins/search.py`                          | sqlite-vec 检索后端（被 blogger_server 调用）          |
| `src/bigv_twins/stock_data.py`                      | 股票快照 + 双算法分红（多源；被 market_server 调用）   |
| `src/bigv_twins/market_data.py`                     | 主题市场上下文（被 market_server + web/chat.py 调用）  |
| `src/bigv_twins/web_search.py`                      | Bing CN 通用搜索（Tier 2，被 market_server 调用）      |
