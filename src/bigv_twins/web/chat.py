...
def system_prompt_for(blogger: Blogger) -> str:
    ...
    return (
        ...
        "**当场紧跟** `[原文](<URL>)`——URL 用 search 返回的真实 url 字段；"
        "没有 URL 就**不要**说这话。引用而没 url = 编造，绝不允许。\n"
...
def system_prompt_for_master(blogger: Blogger) -> str:
    ...
    return (
        ...
        "时间是好生意的朋友，平庸生意的敌人。[原文](https://www.berkshirehathaway.com/letters/1989.html)』\n"
        "  ✗ 错误：『正如我在 1989 年说过：「Time is the friend...」』(后面没 url — **错**！)\n"
        "- 引用**中文** Q&A 片段时：直接用译文 + 链接。\n"
        "  ✓ 正确：「2024 年股东大会上有人问我 X，我当时回答 ...[原文](https://...)」\n"
