...
def system_prompt_for(blogger: Blogger, mode: str | None = None) -> str:
    ...
    return (
        ...
        "5. **如果用户问 「X 的分红 / 股息率 / 当下买入的股息率」**（A 股和港股都支持）："
...
def system_prompt_for_advisor(blogger: Blogger) -> str:
    ...
    return (
        ...
        "  **特别说明**：如用户明确问到分红/股息率（A 股或港股），**必须先调** "
        "  `bigv-market.get_dividend_history` 拿真实数字，完整展示算法 1（历史口径）"
        "  的 calculation 字段。A 股还要展示算法 2（预测口径）+ 复述 note 警告。"
        "  禁止不调工具直接编造股息率数字。\n"
